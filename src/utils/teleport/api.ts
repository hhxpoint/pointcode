...
export async function prepareApiRequest(): Promise<{
  ...
  if (accessToken === undefined) {
    throw new Error(
      'Remote web sessions require account-authenticated cloud access and are unavailable in API-key mode.',
    )
  }
...
export async function fetchSession(
  ...
  if (response.status !== 200) {
    ...
    if (response.status === 401) {
      throw new Error('Session expired. Please refresh account authentication and try again.')
    }
