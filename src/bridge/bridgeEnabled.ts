...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires PointCode account authentication and is unavailable in API-key mode.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires full account authentication. Long-lived tokens are limited to inference-only for security reasons.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. This feature requires account authentication.'
    }
