...
import { hasPersistedProviderProfile, loadProviderProfileIntoProcessEnvIfNeeded } from './utils/providerSetup.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  loadProviderProfileIntoProcessEnvIfNeeded();
  const apiProvider = getAPIProvider();
  // PointCode: Always treat as OpenAI provider to skip Anthropic onboarding
  const isOpenAIProvider = true;
  const hasAnthropicApiKey = Boolean(process.env.ANTHROPIC_API_KEY);
  const hasProviderProfile = hasPersistedProviderProfile();
  ...
  const shouldShowProviderSetup = false; // PointCode: Skip provider setup
  if (shouldShowProviderSetup) {
    const {
      ProviderSetup
    } = await import('./components/ProviderSetup/index.js');
