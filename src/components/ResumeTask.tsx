  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading PointCode sessions...</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying...' : 'Fetching your PointCode sessions...'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
    return <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading PointCode sessions
        </Text>
...
function determineErrorType(errorMessage: string): LoadErrorType {
  ...
  if (message.includes('auth') || message.includes('token') || message.includes('permission') || message.includes('oauth') || message.includes('not authenticated') || message.includes('/model key') || message.includes('/login') || message.includes('console account') || message.includes('403')) {
    return 'auth';
  }
...
function renderErrorSpecificGuidance(errorType: LoadErrorType): React.ReactNode {
  switch (errorType) {
    ...
    case 'auth':
      return <Box marginY={1} flexDirection="column">
          <Text dimColor>Teleport requires account-authenticated cloud access</Text>
          <Text dimColor>
            Configure API access with <Text bold>/model key &lt;api_key&gt;</Text>
          </Text>
        </Box>;
    case 'api':
      return <Box marginY={1} flexDirection="column">
    ...
    case 'other':
      return <Box marginY={1} flexDirection="row">
          <Text dimColor>Sorry, PointCode encountered an error</Text>
        </Box>;
  }
}
