...
function validateProviderEnvOrExit(): void {
  ...
  if (!process.env.OPENAI_API_KEY && !isLocalProviderUrl(request.baseUrl)) {
    // Allow entering the CLI without a key. The user can configure it via
    // /model key <api_key> or /provider key <provider_id> <api_key>.
    return
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (PointCode)`);
    return;
  }
